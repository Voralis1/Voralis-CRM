...
-- ---------------------------------------------------------------------
do $$ begin
  create type user_role as enum ('admin', 'affiliate', 'media_buyer');
...
create table if not exists orders (
  ...
  ip             text,
  user_agent     text,
  affiliate text, sub1 text, sub2 text, sub3 text, sub4 text, sub5 text,
  comment        text,
  ...
  paid_at        timestamptz,                    -- date de paiement du payout au webmaster
  created_at     timestamptz not null default now(),
...
create policy p_orders_aff on orders for select
  using (affiliate_id = my_affiliate_id() or current_role_name() = 'admin');
